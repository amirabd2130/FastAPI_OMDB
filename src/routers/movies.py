...
def get_list_of_movies(limit: Annotated[int, Ge(0)] = 10,
...
def get_a_movie(imdb_id: str | None = None,
                title: str | None = None,
                db: Session = Depends(database.get_db)):
...
def delete_movie_by_imdb_id(imdb_id: str,
                            db: Session = Depends(database.get_db),
                            current_user: schemas.User = Depends(User().get_current_user)):
    return Movie().delete_by_imdb_id(imdb_id, db, current_user)


@router.delete("/cleanup", status_code=status.HTTP_204_NO_CONTENT)
def delete_all_movies(db: Session = Depends(database.get_db),
                      current_user: schemas.User = Depends(User().get_current_user)):
    return Movie().delete_all(db, current_user)
